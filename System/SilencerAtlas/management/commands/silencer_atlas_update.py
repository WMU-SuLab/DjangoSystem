...
def update_silencers(file_path):
    """
    100000/3min
    10-12hours
    """
...
@print_accurate_execute_time
def update_samples_recognition_factors_score(file_path):
    """
    1000/min
    200hours
    """
    ...
    for df in read_csv_n_lines_each_time_by_pandas_yield(file_path, sep='\t', chunk_size=chunk_size):
        ...
        silencer_sample_recognition_factors = list(SilencerSampleRecognitionFactors.objects.filter(
        ...
        SilencerSampleRecognitionFactors.objects.bulk_update(silencer_sample_recognition_factors, fields=['z_score'],
                                                             batch_size=batch_size)


@print_accurate_execute_time
def update_samples_recognition_factors_recognized(file_path):
    """
    1000/15s
    10000/2.5min
    50hours
    ...
    for df in read_csv_n_lines_each_time_by_pandas_yield(file_path, sep='\t', chunk_size=chunk_size):
        ...
        SilencerSampleRecognitionFactors.objects.bulk_update(silencer_sample_recognition_factors, fields=['recognized'],
                                                             batch_size=batch_size)


@print_accurate_execute_time
def update_target_genes(file_path):
    """
    100000/1.5min
    3hours
    """
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if [update_silencer, update_target_gene, update_z_score, update_recognized].count(True) == 0:
            ...
            return update_silencers(file_path)
        elif update_target_gene:
            return update_target_genes(file_path)
        elif update_z_score:
            return update_samples_recognition_factors_score(file_path)
        elif update_recognized:
            return update_samples_recognition_factors_recognized(file_path)
